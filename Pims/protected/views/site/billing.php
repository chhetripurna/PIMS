...
<?php
	...
	echo "<tr><th colspan=3>".CHtml::link('Add Insurance',array('insurance/create','pID'=>$model->patientID))."</th></tr>";
	if($model->insurance)
		{
			echo "<tr><th>Insurance ID</th><th>Provider</th><th>Account Number</th><th>Group Number</th></tr>";
			foreach($model->insurance as $row){
		...
		echo '<tr><th colspan=4><p align=center>This patient does not have insurance</p></th></tr>';
	}

	echo "<tr><th colspan=5><h3><b>Visits</b></h3></th></tr>";
	echo "<tr><th>".CHtml::link("Add Visit",array('visit/create','pID'=>$model->patientID))."</th></tr>";
	echo "<tr><th>Visit ID</th><th>Admit Date</th><th>Discharge Date</th></tr>";
	
foreach($model->visits as $row){
		echo "<tr><td>".CHtml::link(CHtml::encode($row['visitID']),array('/site/visit','id'=>$row['visitID']))."</td><td>".CHtml::encode($row['admitDate'])."</td><td>".CHtml::encode($row['dischargeDate'])."</td></tr>";
	}
	echo "</table>" ?>
</div>
