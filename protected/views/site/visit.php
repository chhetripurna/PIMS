...
<?php
	...
	echo "<tr><th>Charge ID</th><th>Item Charged</th><th>Charge Amount</th><th>Total Charges</th></tr>";

	$total = 0;

foreach($model->billings->charges as $row){
	$total = $total + $row['amount'];
	echo "<tr><td>".CHtml::encode($row['chargeID'])."</td><td>".CHtml::encode($row['item'])."</td><td>".CHtml::encode($row['amount'])."</td><td>".CHtml::encode($total)."</tr>";
	}
